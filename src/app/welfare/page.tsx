  ...
  return (
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">??</span><h1 className="text-2xl font-black text-white">거제시 복지혜택 · 건강보험 절감</h1></div>
        <p className="text-sm text-slate-400 mb-2">업데이트: {data.updatedAt}</p>
        <p className="text-base text-white/70 mb-8 font-medium">{data.summary}</p>
        <h2 className="text-lg font-bold text-slate-700 mb-4">?? 복지 혜택</h2>
        <div className="grid grid-cols-1 gap-4 mb-10">
          {data.welfare?.map((w: any, i: number) => (
            <div key={i} className="bg-white/10 rounded-2xl p-5 border border-white/10 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <div className="font-bold text-white">{w.title}</div>
                <span className={`text-xs px-3 py-1 rounded-full font-bold ${tagColors[w.tag]||"bg-slate-100 text-white/70"}`}>{w.tag}</span>
              </div>
              <div className="text-sm text-white/70 mb-2">{w.desc}</div>
              <div className="text-sm text-white/55 space-y-1"><div>?? {w.target}</div><div>?? {w.amount}</div><div>?? {w.how}</div></div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 mb-10">
          {data.insuranceTips?.map((t: any, i: number) => (
            <div key={i} className="bg-white/10 rounded-2xl p-5 border border-white/10 shadow-sm">
              ...
              <div className="text-sm text-white/55 mb-2">{t.desc}</div>
